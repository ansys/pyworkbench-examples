...
# Define thicknesses for the shell bodies
GEOM.Activate()
BD1 = [i for i in GEOM.GetChildren[Ansys.ACT.Automation.Mechanical.Body](True) if i.Name == 'Surface Body1'][0]
BD1.Activate()
BD1.Thickness=Quantity('1 [mm]')
BD2 = [i for i in GEOM.GetChildren[Ansys.ACT.Automation.Mechanical.Body](True) if i.Name == 'Surface Body2'][0]
BD2.Activate()
BD2.Thickness=Quantity('1 [mm]')
BD3 = [i for i in GEOM.GetChildren[Ansys.ACT.Automation.Mechanical.Body](True) if i.Name == 'Surface Body3'][0]
BD3.Activate()
BD3.Thickness=Quantity('1 [mm]')
BD4 = [i for i in GEOM.GetChildren[Ansys.ACT.Automation.Mechanical.Body](True) if i.Name == 'Surface Body4'][0]
BD4.Activate()
BD4.Thickness=Quantity('1 [mm]')
...
NS_Curve_Edges = [i for i in NS_GRP.GetChildren[Ansys.ACT.Automation.Mechanical.NamedSelection](True) if i.Name == 'NS_Curve_Edges'][0]

# Insert General Axisymmetric under Symmetry folder
SYMM = Model.AddSymmetry()
GEN_AXISYMM = SYMM.AddGeneralAxisymmetric()
GEN_AXISYMM.Location = NS_All_Bodies
GEN_AXISYMM.NodalPlanes = 3
...
MSH_SIZING1.Location = NS_Curve_Edges
MSH_SIZING1.Type=SizingType.NumberOfDivisions
MSH_SIZING1.NumberOfDivisions=4

MSH_SIZING2 = MSH.AddSizing()
MSH_SIZING2.Activate()
MSH_SIZING2.Location = NS_All_Bodies
MSH_SIZING2.ElementSize=Quantity("1.5 [mm]")
...
RMPT1.YCoordinate = Quantity("0 [mm]")
RMPT1.Behavior=LoadBehavior.Rigid

Model.Activate()
RMPT2 = Model.AddRemotePoint()
RMPT2.Location = NS_RemotePoint_Bearing1
RMPT2.Name = "RemotePoint_Bearing1"
RMPT2.YCoordinate = Quantity("0 [mm]")
RMPT2.Behavior=LoadBehavior.Rigid
...
RMPT3.Behavior=LoadBehavior.Rigid
...
RMPT6.Behavior=LoadBehavior.Rigid
...
# Clean and solve Modal analysis for 2D Axisymmetric Model without rotation
...
# Clean and solve Modal analysis for 2D Axisymmetric Model with rotation of 50000 rpm
...
# Clean and solve Campbell diagram analysis of the 2D Axisymmetric Model
...
# Clean and solve unbalance response of the 2D Axisymmetric Model
...
#TOT_DEF4_1 = [x for x in ExtAPI.DataModel.Tree.AllObjects if x.Name == 'Total Deformation1@1666.7Hz'][0]
TOT_DEF4_1.Activate()
TOT_DEF4_2.Activate()
